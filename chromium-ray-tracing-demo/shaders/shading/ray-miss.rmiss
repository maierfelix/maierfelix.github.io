...
void main() {
  const vec3 rd = normalize(gl_WorldRayDirectionEXT.xyz);
  const float theta = acos(clamp(rd.y, -1.0, 1.0));
  const vec2 uv = vec2((PI + atan(rd.z, rd.x)) * (1.0 / TWO_PI), theta * (1.0 / PI));

  float misWeight = 1.0;
  if (Ray.depth > 0) {
    const float c_pdf = texture(sampler2DArray(EnvConditionalArray, NearestSampler), vec3(uv, Settings.envTextureIndex)).y;
    const float m_pdf = texture(sampler2DArray(EnvMarginalArray, NearestSampler), vec3(uv.y * 0.5, 0.0, Settings.envTextureIndex)).y;
    const float pdf = c_pdf * m_pdf;
    const float lightPdf = (pdf * Settings.envResolution) / (2.0 * PI * PI * sin(theta));

    misWeight = powerHeuristic(Ray.scatterDirectionAndPDF.w, lightPdf);
  }

  const vec3 hdrColor = texture(sampler2DArray(EnvironmentArray, LinearSampler), vec3(uv, Settings.envTextureIndex)).rgb;
  const vec3 radiance = misWeight * hdrColor * Ray.throughput.rgb * Settings.envHdrMultiplier;

  Ray.radianceAndDistance = vec4(radiance, -1.0);
}
